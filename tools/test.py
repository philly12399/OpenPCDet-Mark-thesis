...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False):
    ...
    eval_utils.eval_one_epoch(
        ...
        result_dir=eval_output_dir, save_to_file=args.save_to_file, eval_range=args.eval_range
    )
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    while True:
        ...
        model.load_params_from_file(
            filename=cur_ckpt, logger=logger, to_cpu=dist_test)
        model.cuda()
...
def main():
    ...
    with torch.no_grad():
        if args.eval_all:
            repeat_eval_ckpt(model, test_loader, args, eval_output_dir,
                             logger, ckpt_dir, dist_test=dist_test)
        else:
            eval_single_ckpt(model, test_loader, args, eval_output_dir,
                             logger, epoch_id, dist_test=dist_test)


# def unify_difficulty(dets):
#     import copy
#     dets = copy.deepcopy(dets)
#     for det in dets:
#         for i in range(len(det['difficulty'])):
#             if det['difficulty'][i] != -1:
#                 det['difficulty'][i] = 0
#     return dets
