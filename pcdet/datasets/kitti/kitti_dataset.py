...
class KittiDataset(DatasetTemplate):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                annotations['bbox'] = np.concatenate([obj.box2d.reshape(
                    1, 4) for obj in obj_list], axis=0) if len(obj_list) > 0 else np.array([]).reshape(0, 4)
                annotations['dimensions'] = np.array(
                    [[obj.l, obj.h, obj.w] for obj in obj_list]).reshape(-1, 3)  # lhw(camera) format
                annotations['location'] = np.concatenate(
                    [obj.loc.reshape(1, 3) for obj in obj_list], axis=0) if len(obj_list) > 0 else np.array([]).reshape(0, 3)
                ...
                if count_inside_pts:
                    if len(obj_list) > 0:
                        ...
                        annotations['num_points_in_gt'] = np.ones(
                            0, dtype=np.int32)
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_dict['dimensions'] = pred_boxes_camera[:, 3:6].reshape(
                -1, 3)
            pred_dict['location'] = pred_boxes_camera[:, 0:3].reshape(
                -1, 3)
