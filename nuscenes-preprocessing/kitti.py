...
class KittiDB:
    ...
    def __init__(self,
        ...
        self._kitti_fileext = {
            'calib': 'txt', 'image_2': 'png', 'label_2': 'txt', 'velodyne': 'bin'}

        # Grab all the expected tokens.
        self._kitti_tokens = {}
        for split in splits:
            split_dir = osp.join(self.root, 'label_2')
            _tokens = os.listdir(split_dir)
            _tokens = [t.replace('.txt', '') for t in _tokens]
            _tokens.sort()
        ...
        self.kitti_to_nu_lidar_inv = Quaternion(
            axis=(0, 0, 1), angle=np.pi / 2).inverse
    ...
    def get_filepath(token: str, table: str, root: str = '/data/sets/kitti') -> str:
        ...
        kitti_fileext = {'calib': 'txt', 'image_2': 'png',
                         'label_2': 'txt', 'velodyne': 'bin'}
        ...
        if token.startswith('test_') and table == 'label_2':
            ...
            filepath = osp.join(root, table,
                                '{}.{}'.format(filename, ending))
    ...
    def get_transforms(token: str, root: str = '/data/sets/kitti') -> dict:
        ...
        velo_to_cam = np.array(lines[5].strip().split(' ')[
                               1:], dtype=np.float32)
    ...
    def get_pointcloud(token: str, root: str = '/data/sets/kitti') -> LidarPointCloud:
        ...
        pc = LidarPointCloud(np.fromfile(
    ...
    def get_boxes(self,
        ...
        with open(KittiDB.get_filepath(token, 'label_2', root=self.root), 'r') as f:
            for line in f:
                ...
                quat_box = Quaternion(
                    axis=(0, 1, 0), angle=yaw_camera) * Quaternion(axis=(1, 0, 0), angle=np.pi/2)
                ...
                box.rotate(Quaternion(
                    matrix=transforms['velo_to_cam']['R']).inverse)
                ...
                boxes.append(box)
                truncs.append(parsed_line['truncation'])
    ...
    @staticmethod
    def box_to_string(box: Box,
                      bbox_2d: Tuple[float, float, float,
    ...
    def project_pts_to_image(self, pointcloud: LidarPointCloud, token: str) -> np.ndarray:
        ...
        points_fov = view_points(
            pc_image.points[:3, :], transforms['p_combined'], normalize=True)
        points_fov[2, :] = depth
    ...
    def render_sample_data(self,
        ...
        boxes = self.get_boxes(
            token, filter_classes=filter_classes, max_dist=max_dist)
        ...
        if sensor_modality == 'lidar':
            ...
            if with_anns:
                for box in boxes:
                    ...
                    box.render(ax, view=view_3d, colors=(
                        color, color, 'k'), linewidth=box_linewidth)
                ...
                if render_2d:
                    ...
                    for box, name in zip(boxes_2d, names):
                        ...
                        ax.plot([box[0], box[0]], [box[1], box[3]],
                                color=color, linewidth=box_linewidth)
                    ...
                    for box in boxes:
                        ...
                        box.rotate(Quaternion(
                            matrix=transforms['velo_to_cam']['R']))
                        # In KITTI camera frame, un-rectified.
                        box.translate(transforms['velo_to_cam']['T'])
