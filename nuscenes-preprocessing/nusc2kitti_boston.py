...
class KittiConverter:
    ...
    def nuscenes_gt_to_kitti(self) -> None:
        ...
        if self.convert_labels:
            ...
            sample_tokens, track_list, track_timestamps = self._split_to_samples(
                boston_scenes)
            ...
            pickle.dump(track_timestamps, open(os.path.join(
                self.nusc_kitti_dir, 'training', 'track_timestamps.pkl'), 'wb'))
    ...
    def _split_to_samples(self, split_tokens: List[str]) -> List[str]:
        ...
        for scene in split_tokens:
            ...
            track_timestamps.append(ld_timestamp)
            ...
            for _ in ld_tokens:
                ...
                cnt += 1
        return samples, track_list, track_timestamps
...
def parse_args():
    ...
    parser.add_argument(
        '--nusc_version', type=str, default=None)
    parser.add_argument(
...
if __name__ == '__main__':
    args = parse_args()
    nusc_version = 'v1.0-trainval' if args.nusc_version is None else args.nusc_version
    split = 'train' if args.split is None else args.split
    kc = KittiConverter(
